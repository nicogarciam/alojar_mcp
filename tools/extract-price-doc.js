...
async function main() {
    ...
    const pricesEntries = Object.entries(paths).filter(([p]) => /accommodation_prices/i.test(p) || Object.values(paths[p]).some(op => {
        // también buscar operaciones con tags o summary que contengan accommodation_prices
        return Object.values(op || {}).some(v => {
            if (!v) return false;
            if (v.tags && v.tags.join(' ').toLowerCase().includes('accommodation_prices')) return true;
            if (v.summary && String(v.summary).toLowerCase().includes('accommodation_prices')) return true;
            if (v.description && String(v.description).toLowerCase().includes('accommodation_prices')) return true;
            if (v.description && String(v.description).toLowerCase().includes('price_grid')) return true;
            return false;
        });
    }));
