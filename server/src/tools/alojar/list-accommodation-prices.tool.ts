...
export function registerListAccommodationPricesTool(server: McpServer) {
    server.registerTool(
        'list_accommodation_prices',
        {
        ...
        async (inputRaw, extra): Promise<CallToolResult> => {
            ...
            const normalizeArray = (v: any): string[] | undefined => {
                ...
                if (s.startsWith('[')) {
                    try { const parsed = JSON.parse(s); if (Array.isArray(parsed)) return parsed.map((x: any) => String(x)); } catch { }
                }
                // split por comas
