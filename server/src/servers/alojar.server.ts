...
export class AlojarServer {
    ...
    private setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request: CallToolRequest) => {
            if (request.params.name === 'check_availability') {
                try {
                    const { hotel_id, date_from, date_to, pax } = request.params.arguments as any;
                    // Validar parámetros
                    ...
                    const apiResponse = await this.availabilityService.quickAvailability(
                        2,
                        date_from,
                        date_to,
                        pax
     ...
     */
    private formatAvailabilityResponse(response: any): string {
        ...
        if (response.availables_count > 0) {
            ...
            response.availables.forEach((acc: any) => {
        ...
        if (response.options && response.options.length > 0) {
            ...
            response.options.forEach((option: any) => {
                ...
                option.available.forEach((period: any) => {
